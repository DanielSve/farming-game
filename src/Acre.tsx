...
const Acre = ({
  ...
  useEffect(() => {
    value !== -1 &&
      setCrops(
        crops.map((crop, i2) => (i2 === index ? allCrops[value] : crop))
      );
    value !== -1 &&
      setPlayer({ ...player, balance: player.balance - allCrops[value].cost });
  ...
  return (
    <div className='acre' style={{ backgroundColor: color }}>
      {turnDone ? (
        ...
        <div className='Results'>Yield ${results[index].yield}</div>
      )}
    </div>
  );
};
